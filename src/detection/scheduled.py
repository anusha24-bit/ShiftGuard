...
def detect_scheduled_shifts(df, calendar_df, feature_cols, window_size=60, alpha=0.05):
    ...
    for event_date in event_dates:
        ...
        pre_data = df.iloc[pre_start:event_idx][feature_cols].dropna(axis=1).values
        post_data = df.iloc[event_idx:post_end][feature_cols].dropna(axis=1).values
        ...
        pre_data = pre_data[:, :min_cols]
        post_data = post_data[:, :min_cols]

        # KS test per feature
        ks_pvalues = []
        ks_stats = []
        usable_cols = [c for c in feature_cols if c in df.columns][:min_cols]
        ...
        for i in range(min_cols):
            ...
            stat, pval = ks_2samp(pre_col, post_col)
        ...
        # Severity score (0-5)
        severity = min(5, int(significant_ratio * 10))

        # Get event names for this date
        event_names = high_events[high_events['date'] == event_date]['event_name'].tolist()

        # Shift detected if >20% of features show significant change
        if significant_ratio > 0.20:
            shifts.append({
                'datetime_utc': str(event_date),
                'type': 'scheduled',
                'severity': severity,
                'significant_features_ratio': round(significant_ratio, 3),
                'mean_ks_stat': round(np.mean(ks_stats), 4),
